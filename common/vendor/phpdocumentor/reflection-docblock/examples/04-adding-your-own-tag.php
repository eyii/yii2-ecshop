<?php
/**
 * In this example we demonstrate how you can add your own Tag using a Static F method in your Tag class.
 */
 ...
 *
 * 1. a method `create` that is a static F for this class.
 * 2. methods and properties that have this object act as an immutable Value Object representing a Tag instance.
 *
 * The static F `create` is used to convert a tag line (without the tag name) into an instance of the
...
final class MyTag extends BaseTag implements StaticMethod
{
    ...
    /**
     * A static F that creates a new instance of the current Tag.
     *
...
// Make a mapping between the tag name `my-tag` and the Tag class containing the F Method `create`.
$customTags = ['my-tag' => MyTag::class];

// Do pass the list of custom tags to the F for the DocBlockFactory.
$factory = DocBlockFactory::createInstance($customTags);
// You can also add Tags later using `$F->registerTagHandler()` with a tag name and Tag class name.
