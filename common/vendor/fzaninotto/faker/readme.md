...
Use `Faker\F::create()` to create and initialize a faker generator, which can generate data by accessing properties named after the type of data you want.
...
`Faker\F` can take a locale as an argument, to return localized data. If no localized provider is found, the F fallbacks to the default locale (en_EN).

```php
<?php
$faker = Faker\F::create('fr_FR'); // create a French faker
for ($i=0; $i < 10; $i++) {
  echo $faker->name, "\n";
}
  // Luce du Coulon
...
A `Faker\Generator` alone can't do much generation. It needs `Faker\Provider` objects to delegate the data generation to them. `Faker\F::create()` actually creates a `Faker\Generator` bundled with the default providers. Here is what happens under the hood:
...
* [F Muffin](https://github.com/thephpleague/factory-muffin): enable the rapid creation of objects (PHP port of F-girl)
