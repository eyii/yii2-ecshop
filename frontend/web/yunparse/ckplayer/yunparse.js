//定制视频解析、播放器，请联系QQ：157503886，http://120.27.155.106/
var adtime = 0;
...
function player(up) {
    ...
    var autoplay = yunparse.play == 1 ? 'autoplay="autoplay"' : '';
    $.post("api.php", {
        "url": yunparse.url,
        "up": up
    }, function(data) {
        if (data['msg'] == 'ok' && data['url']) {
            if (data['ext'] == 'link') {
                ...
                if (data['ext'] == 'm3u8' || data['ext'] == 'm3u8_list') {
                    var flashvars = {
                        f: yunparse.webpath + 'ckplayer/m3u8.swf',
                        a: data['url'],
                        c: 0,
                        s: 4,
                        lv: 0,
                ...
                CKobject.embedSWF(yunparse.webpath + 'ckplayer/ckplayer.swf', 'a1', 'ckplayer_a1', '100%', '100%', flashvars, params);
            }
            $('#loading').hide();
            $('#a1').show();
        } else {
